...
class fine_UNETR(nn.Module):
    def __init__(
        ...
        self.encoder1 = CEE(in_chans=16, embed_dim=32, double_branch=1, use_att=0)
        self.encoder2 = CEE(in_chans=32, embed_dim=64, double_branch=1, use_att=0)
        self.encoder3 = CEE(in_chans=64, embed_dim=128, double_branch=1, use_att=0)
        self.encoder4 = CEE(in_chans=128, embed_dim=256, double_branch=1, use_att=0)
        self.encoder5 = nn.Conv3d(in_channels=256, out_channels=768, kernel_size=1, stride=1, padding=0, groups=256)
    ...
    def forward(self, x_in):
        ...
        x_to_encoder = self.pfc(x_in)       # 4 1 32 32 32  -> 4 16 32 32 32
        x_en1 = self.encoder1(x_to_encoder) # 1 16 32 32 32 -> 1 32 16 16 16
        hidden_states_out.append(x_en1)     # [1 32 16 16 16]
        x_en2 = self.encoder2(x_en1)        # 1 16 16 16 16 -> 1 64 8 8 8
        hidden_states_out.append(x_en2)     # [1 32 16 16 16, 1 64 8 8 8]
        x_en3 = self.encoder3(x_en2)        # 1 64 8 8 8 -> 1 128 4 4 4
        hidden_states_out.append(x_en3)     # [1 32 16 16 16, 1 64 8 8 8, 1 128 4 4 4]
        x_en4 = self.encoder4(x_en3)        # 1 128 4 4 4 -> 1 256 2 2 2
        x_en5 = self.encoder5(x_en4)        # 1 256 2 2 2 -> 1 768 2 2 2 最后一层编码结果 也是最深层 空间分辨率最小的编码结果
        x = x_en5.flatten(2).transpose(1, 2)# 4 8 768
        ##-----fine unet encoding-----
        #------fine unet encode sub_network------
        enc1 = x_to_encoder # self.encoder1(x_in)
        # x2 = hidden_states_out[3]
        enc2 = x_en1 # self.encoder2(self.proj_feat(x2, self.hidden_size, self.feat_size))
        # x3 = hidden_states_out[6]
        enc3 = x_en2 # self.encoder3(self.proj_feat(x3, self.hidden_size, self.feat_size))
        # x4 = hidden_states_out[9]
        enc4 = x_en3 # self.encoder4(self.proj_feat(x4, self.hidden_size, self.feat_size))
        dec4 = self.proj_feat(x, self.hidden_size, self.feat_size) # 4 8 768 -> 4 768 8 8 8
